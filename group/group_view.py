...
@group_bp.route('/group_chat/<group_id>/<sender_id>', methods=['GET', 'POST'])
def group_chat(group_id, sender_id):
    ...
    if request.method == 'POST':
        ...
        new_message = GroupMessage(group_id=group_id,
        	sender_id=sender_id, content=content)
        storage.new(new_message)
        storage.save()

    messages = storage.all(GroupMessage).values()
    group_messages = [message for message in messages if message.group_id == group_id]
...
@group_bp.route('/delete_group/<group_id>/<hub_id>', methods=['GET', 'POST'])
def delete_group(group_id, hub_id):
	...
	return redirect(url_for('group.create_group', hub_id=hub_id))